# Algorithm Design Specification: Version v1.2.0-4fea7cba

This document outlines the architecture, mathematical formulas, and quant mechanics of the ASX Screener Algorithm for version **v1.2.0-4fea7cba**.
...
*   **State-Dependent Thresholds**:
    ...
    | Accumulation Volume Spike (`accum_spike_threshold`) | `1.10` | `1.30` |
...
For this build, the base version is **v1.2.0** and the configuration fingerprint is **4fea7cba**. This guarantees parameter-to-build binding, meaning any modification to thresholds in `strategy_config.py` automatically updates the version string.
...
3.  **Volume Ratio**: The current volume relative to its 20-day historical average.
    ...
    ```
4.  **ATR Percentile**: Measured using the standard Average True Range over 14 days relative to price.
    ```math
...
1.  **Momentum Breakout ▶**
    ...
    *   *Auxiliary Conditions*: At least 2 of the following 3 must hold:
        1.  `ma_bullish`: $\text{MA}_{5} > \text{MA}_{10} > \text{MA}_{20}$ AND $\text{Close}_{t} > \text{MA}_{5}$ AND $\text{Close}_{t} > \text{MA}_{60}$.
        2.  `breakout_vol_spike`: Volume percentile $Rank\% > \text{vol\_percentile\_threshold}$ (0.70 default, 0.75 low risk) calculated over a 61-day window including today (`volume.iloc[-61:]`).
...
2.  **Accumulation Zone ◉**
    *   *Conditions*:
        ...
        2.  `accum_vol_spike`: $\text{Volume Ratio} > \text{accum\_spike\_threshold}$ (1.10 default, 1.30 low risk).
...
```math
\text{Base Score} = (\text{Avg Change} \times 6) + (\text{Avg Vol Ratio} \times 15) + (\text{Momentum Ratio} \times 80) + (\text{Accumulation Ratio} \times 50)
```
where:
```math
\text{Momentum Ratio} = \frac{N_{\text{momentum}}}{\text{constituent\_stocks\_count}}
```
```math
\text{Accumulation Ratio} = \frac{N_{\text{accumulation}}}{\text{constituent\_stocks\_count}}
```
...
*   **Macro Bonus**: Adjusts for treasury yield trends and AUD currency strength using sector multipliers configured in `strategy_config.py`:
    ...
    *   *Travel*: positive AUD coefficient (`macro_bonus_travel_multiplier` = 5.0).
...
*   **Executable Returns (Next Open-to-Close)**:
    ...
    ```
*   **Aligned Benchmark Returns for Executable Mode**:
    Calculated starting from the next trading day's Open price to remain consistent with stock executable returns:
    ```math
    \text{Return}_{\text{bench\_exec}, Nd} = \frac{\text{Close}_{\text{bench}, t+N} - \text{Open}_{\text{bench}, t+1}}{\text{Open}_{\text{bench}, t+1}} \times 100
    ```
*   **Transaction Cost Deduction**:
    ...
    ```
    ```math
    \text{Alpha}_{\text{net}, \text{exec}} = (\text{Return}_{\text{exec}, Nd} - \text{Return}_{\text{bench\_exec}, Nd}) - \text{Double Cost}
    ```
